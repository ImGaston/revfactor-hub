...
- Important app area: `app/(authenticated)/` holds the authenticated dashboard, clients, listings, tasks, adjustments, roadmap, GHL (`/ghl`, GoHighLevel connection), onboarding, financials, settings, and knowledge routes. `app/a/[token]/` is the public Adjustments share card (no login).
