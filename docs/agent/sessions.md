...
- The first Vercel Preview was rejected before build because the Hub project does not allow the proposed third every-minute cron. Removed that unsupported cron registration, kept the protected worker endpoint, chained up-to-five daily market jobs after the existing Stripe cron, lowered PriceLabs inventory-job priority so due provider work folds in that evidence, and made editor refresh drain one job immediately. This preserves automatic and on-demand operation without relinking to the unrelated onboarding project; higher-frequency NWS monitoring remains a hosting-plan follow-up.
- Production verification loaded the authenticated Market Signals queue and ran Washington's first multi-source refresh. NWS activated and completed; Ticketmaster correctly exposed an HTTP 400 source-health failure. Exact-query reproduction found Ticketmaster rejects ISO milliseconds in `startDateTime`/`endDateTime` even though the values are valid ISO 8601, so the adapter now emits the provider's strict `YYYY-MM-DDTHH:mm:ssZ` format with regression coverage.
- The strict-date hotfix reached production and Ticketmaster began returning rows. The same live refresh then exposed duplicate Ticketmaster records with one canonical title/date/venue fingerprint racing inside a six-row persistence batch. Provider batches now contain distinct fingerprints, deferring same-event provider records to later batches so parallelism remains bounded and every source record is retained without violating canonical-event uniqueness.
...
# 2026-08-28 — Listing lifecycle tracking, monthly summary for HostPricing, client PMS/Vrbo

- Migration 086 (applied to prod): lifecycle DATE columns on `listings`, `pms_name`/`has_vrbo` on `clients`, deactivation-stamp BEFORE trigger, `listing_status_events` + SECURITY DEFINER history trigger, date backfill (26/34 inactive got a `deactivated_date` from Stripe `canceled_at` or client `ending_date`; 8 left NULL for manual entry — e.g. the Alexis Guindi listings), synthetic baseline events (290), and the `monthly_summary` permission resource (view: admin, hostpricing).
- New `/monthly-summary` page + sidebar item + `lib/monthly-summary.ts`; listing dialog gained the three date fields; controlling a `setup` adjustment auto-fills setup dates (only-if-empty, admin client behind the `adjustments:control` check).
- Unified client-inactive behavior via `clientStatusPatch()` in `lib/clients.ts` — the onboarding board path now also stamps/clears `ending_date`. Fixed in passing: editing a listing from `/listings` silently reset `status` to active (the form never loaded it).
...
- Consolidated the four unresolved RF-AUTO-001 commercial policies into `docs/ghl/RF-AUTO-001_FEDE_COMMERCIAL_DECISIONS.md`; the packet is decision-only and does not authorize Stripe Test, merge, deployment, migration, GHL/Assembly wiring, or any production effect.
- Federico explicitly approved decisions 1–4 as recommended on 2026-08-27: billing timing, mandatory re-signing for quantity changes, fail-closed tax treatment pending professional determination, and Federico as the initial sole exception approver. The approval resolves policy choices only and does not authorize implementation or external effects.
- Federico selected `start.revfactor.io` as the future canonical onboarding URL while keeping `onboarding.revfactor.io` active during migration. This records the name only; DNS, funnel attachment, redirect, client distribution, and cutover remain blocked pending end-to-end test evidence and separate approval.
...
- Parent review hardened the boundary before integration: canonical initial Invoice/PaymentIntent amount/currency/status, Subscription latest-invoice and exact active/trial state, environment/account binding, one normalized line shape, durable conflict audit, immutable agreement/attempt authority, service-billing transitions, run-stable Assembly dedupe, and a database final-handoff trigger.
- Ran the real local 087 followed by revised 088 in ephemeral socket-only PostgreSQL clusters: forward, transactional rollback, forward-again, 20 concurrent claims, success/conflict replay, out-of-order failure, RLS/grants/function signatures, immutability, billing transitions, outbox atomicity, and Assembly final gate all passed. No production or shared database was contacted.

# 2026-09-03 — RF-AUTO-001 multi-business onboarding implementation

- Created isolated branch `codex/rf-auto-001-multi-business` on current canonical main, preserving PR #45 and the dirty primary checkout. Rebased the four approved PR #45 commits and stacked the approved inert checkout foundation.
- Added server-owned group/account freezing for single-business and separate-business-per-property paths. Regular pricing is `$350`; an allowlisted referral code applies `$320` to the whole group; the one `$150` fee allocates exactly across 1–5 accounts.
- Added unwired V2 GHL page code and Worker routes. The Durable Object persists one group plus ordered account stages, creates one stable GHL Opportunity per account, supplies `opportunityId` to the native template API, fences every post-await transition, and returns an HMAC-authenticated resume token plus server-owned next action. Existing client-facing code and Worker remain unchanged.
- Created the inert GHL pipeline `RevFactor Onboarding Accounts — DRAFT` (`FI2x1Yz3C9hcuGjoZibx`) with six stages, seven Opportunity commercial fields, and unpublished Opportunity-native standard/referral contract versions (`6a9a1854ea613131e9b35b95`, `6a9a19b9ea613131e9b37b7c`). Both contracts use Opportunity merge content and keep the native client signature. No contact or document was generated.
- Extended signed checkout entitlements and provider truth with group, billing-account, and Opportunity identity. The price book now allowlists standard/referral recurring prices plus `$150/$75/$50/$37.50/$30` one-time allocation prices.
- Renamed the checkout migration to a unique timestamp after the canonical migration tip and added a second unapplied group/account migration. The database links each run listing and Hub listing to its billing account and blocks Assembly until every account is current, exception-free, payment-verified, and mapped to the consolidated run.
- Verification: 48/48 Cloudflare Worker/Durable Object tests, 29/29 server-checkout tests, TypeScript, Wrangler staging dry-run, inline-script parse, and disposable PostgreSQL canonical-base → checkout → multi-business forward/rollback/forward-again rehearsal passed. No Worker deployment, migration application, GHL publication, Stripe resource/card, Hub/Assembly write, domain change, or client effect occurred.
