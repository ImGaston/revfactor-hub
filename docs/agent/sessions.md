...
## 2026-09-11 — Scrolling dialogs keep their glass background

The Financials "Revisar registros" modal lost its panel background once scrolled: `glass-chrome` paints the dialog surface in an absolute `::before` sized to the visible box, so a `DialogContent` that is itself the scroll container scrolls the background away. Moved scrolling into an inner `min-h-0 overflow-y-auto` body under a `flex flex-col` DialogContent in the two Financials dialogs and in the nine other dialogs using the same pattern (add-listing, listing/client settings, task, adjustment, project detail, link/change subscription, bank import), keeping headers and footers pinned. `post-detail-dialog` was left alone (own inner scroll, `p-0`). Typecheck green; verified the Financials review and New Adjustment dialogs in the logged-in local session. Rule recorded in `conventions.md` UI section. The orphaned local `pnpm dev` (hung in an uncaught-exception loop) was restarted via the app preview during verification.
...
## 2026-09-23 — Unified Airbnb calendar button
Adjustments' "Airbnb calendar" built the multicalendar URL from `listing_id` (the PriceLabs ID), opening the wrong listing. Moved listing deep-link helpers (`pricelabsUrl`, `airbnbRoomUrl`, new `airbnbCalendarUrl` / `airbnbEditorUrl` / `airbnbIdFromLink`) from `lib/adjustments.ts` to `lib/listing-links.ts`, all Airbnb links derived from `airbnb_link` only. New shared `AirbnbCalendarButton` used on the listing page, adjustment detail and public `/a/[token]` card (listing page's "Calendar" is now "Airbnb calendar"). `airbnbRoomUrl` also lost its `listing_id` fallback. Typecheck and 464 lib tests pass.

## 2026-09-25 — Reservations: negative client/listing filters
Client and listing comboboxes on `/reservations` got an "Is / Is not" toggle; "Is not" sets `xclient=1` / `xlisting=1` so the table, CSV export, stats header and saved views show everything except the selected client/listing (unmapped NULL rows stay in). `applyReservationFilters` now sends all OR groups (exclusions + search) as one nested `or=` param. Migration `20260925120000_reservation_stats_exclusions.sql` replaces `reservation_page_stats` with two extra defaulted flags; the app only sends them when excluding, and the migration was applied to `revfactorHub` on 2026-09-25 (single 8-arg overload; anon has no EXECUTE; include + exclude counts sum to the booked total). Typecheck and view tests pass; not visually verified.
