...
Client and listing comboboxes on `/reservations` got an "Is / Is not" toggle; "Is not" sets `xclient=1` / `xlisting=1` so the table, CSV export, stats header and saved views show everything except the selected client/listing (unmapped NULL rows stay in). `applyReservationFilters` now sends all OR groups (exclusions + search) as one nested `or=` param. Migration `20260925120000_reservation_stats_exclusions.sql` replaces `reservation_page_stats` with two extra defaulted flags; the app only sends them when excluding, so it stays compatible with the old signature until the migration is applied (not applied in this session). Typecheck and view tests pass; not visually verified.
