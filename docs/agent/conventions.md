...
- Sidebar sections live in `lib/navigation.ts` (`NAV_ITEMS`), not in the sidebar component. Each has a stable `key` persisted in `nav_item_settings.item_key` — never rename a key (rename `title`/`href` instead). Add new modules there; the folder config in Settings → Navigation is shared team config, and permission filtering still runs per viewer before the tree is built. Folder icons are a curated name→lucide map (`NAV_GROUP_ICONS`); the DB stores only the name.
...
- Client churn fields (`clients.ending_reason_tags`, `clients.ending_note`, migration 048) are super_admin-only via app-layer gating, same pattern as `billing_amount`: RLS rides on `clients:view`, so every loader that selects them must null them for non-super_admin, and `updateClientAction` strips them from non-super_admin input. (`clients.pms_name`/`has_vrbo` are NOT financial — no gating.)
- **Status values** (2026-09-09): `clients.status ∈ active|onboarding|inactive|test`, `listings.status ∈ active|inactive|test` (DB CHECKs, mirrored by `CLIENT_STATUSES`/`LISTING_STATUSES` in `lib/status.ts`). `test` is internal test data: it stays visible everywhere in the hub (lists, detail, settings, pickers, command palette, PriceLabs sync and Report Builder ingest keep running for it) but **every aggregation must exclude it** — dashboard counts, monthly summary/evolution, monthly pacing, wins detection, financials, Stripe auto-link. Write `.neq("status", TEST_STATUS)` (or filter by `isTestStatus` in JS when the data comes from `report_listings`/pickup rows) and import the constant so `rg TEST_STATUS` audits coverage; queries that already use `.eq("status", "active")` exclude it for free. Render status with `components/status-badge.tsx` (`StatusBadge`) — do not redeclare variant/color maps. A client set to `test` cascades only its `active` listings to `test` (`listingCascadeForClientStatus`, shared by Settings and onboarding); linking Assembly never changes a test client's status. Transitions into or out of `test` never stamp or clear `deactivated_date` (the 086 trigger returns early) and `clientStatusPatch` treats it like any non-inactive status.
- Client status changes must go through `clientStatusPatch()` in `lib/clients.ts` (used by `updateClientAction` and onboarding's `updateClientStatus` so they can't drift): going inactive stamps `ending_date` (falls back to today), leaving inactive clears `ending_date`/churn fields. Listing lifecycle dates (migration 086) are stamped by DB triggers, so app code that flips `listings.status` must NOT also write `deactivated_date` — the trigger stamps on `active→inactive`, clears on reactivation, and `record_listing_status_event` appends history for every path including admin-client writes. In monthly-summary math, a missing `initial_setup_date` means carried-over (counts as active, never as new) and a missing `deactivated_date` on an inactive listing excludes it from counts — disclose these buckets in UI, never approximate from `created_at`/`updated_at` (the PriceLabs sync clobbers `updated_at` hourly).
...
- Airbnb cancellation foundation (migration 091): an active listing with `client_id IS NULL` is Blackbird; a non-NULL client is RevFactor. Never create a fake Blackbird client. `default_cancellation_policy` and `timezone` stay NULL until positively inventoried and NULL blocks later automation. Portfolio Adjustments always require a client; single-listing Adjustments must exactly mirror the referenced listing client, including NULL for Blackbird. The database constraint/triggers are authoritative; app validation is only feedback. See `docs/airbnb-seasonal-cancellation-foundation.md`.
...
- **Machine-to-machine auth** (external consumers, no Supabase session) uses `lib/api-auth.server.ts`: `Authorization: Bearer rvf_live_<64 hex>`, resolved by SHA-256 digest against `api_keys` with one indexed lookup, scoped via `API_SCOPES` (`leads:read` or `market-map:read`). 401 for missing/invalid/revoked, 403 for a valid key missing the scope. Keys are issued and revoked with `scripts/create-api-key.ts` / `revoke-api-key.ts` — Hub stores only the digest, while the consumer stores the plaintext in its server-only environment. This is distinct from the inbound webhooks (`WEBHOOK_SECRET`, `SCHEDULER_WEBHOOK_SECRET`) and cron (`CRON_SECRET`), which stay static shared secrets.
...
- Adjustment type visibility is per creator group (`AdjustmentTypeGroup` = `internal | hostpricing | agent`) via `adjustmentTypeOptions(group, currentType, settings)`. Human groups default to visible when a type has no settings row; `agent` is fail-closed (column default FALSE, missing row = disabled, `AGENT_DEFAULT_TYPES` fallback while settings load). The dialog picks the group from the origin select (`agent` origin → Agent column) and clears a type that the new group hides. Agent processes writing `adjustments` directly should respect `agent_enabled` — the server itself still accepts any valid type.
...
- `/listings` treats `listings.status` and `clients.status` independently. Its
  ...
  status badge. The default "Active" filter (and the Settings > Listings
  "active" tab) also matches `test` listings via `matchesListingStatus` in
  `lib/listing-status.ts`; the explicit "Test" option isolates them and
