...
- Optional LangSmith observability is deliberately narrower than Hub observability. `lib/agent-studio-langsmith.server.ts` can trace only the built-in synthetic client, only the Pricing & Performance flow pilot, only without a frozen snapshot, and never when `VERCEL_ENV=production` (or a local production build). It rejects personal access tokens and requires a workspace-scoped service key, workspace id, and the exact `revfactor-agent-studio-sandbox` project. Keep `LANGSMITH_TRACING=false`; the server opts eligible runs in with `traceable(..., { tracingEnabled: true })`, preventing global auto-tracing of real-client runs. The resulting trace id is stored with the Hub run and shown in the Trace inspector. `pnpm langsmith:seed` creates an idempotent eight-case synthetic dataset after local credentials are configured.
...
LangSmith is optional and external: enabling it sends the eligible synthetic question, structured output, and observable LangGraph steps to the configured LangSmith workspace. Do not add LangSmith variables to Vercel Production or relax the code boundary until privacy, retention, and data-processing terms are separately approved. LangSmith is not the source of truth for Hub runs, costs, approvals, or audit history.
