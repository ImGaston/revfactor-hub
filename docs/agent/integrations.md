...
- National Weather Service: implemented authoritative US watches/warnings/advisories through `api.weather.gov/alerts/active?point=...`. NWS has no user account or API key; `NWS_USER_AGENT=(hub.revfactor.io, info@revfactor.io)` identifies the caller. The adapter runs every 15 minutes, maps official severity to an auditable materiality floor (Extreme 85, Severe 70, Moderate 50, Minor 25), and uses authority tier 1. It does not cover non-US listings.
