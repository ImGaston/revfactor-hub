...
### 2026-09-11 — Finance scorecard MRR observations

`syncStripeData` captures an MRR observation after a complete successful subscription mirror update, before payout reconciliation. It resolves discount/coupon expansions only for discounted subscriptions, loads full item pages, normalizes licensed recurring prices (including `unit_amount_decimal`) and records customer/client linkage at observation time. Active and past_due are included; test clients and non-recurring items are excluded. Unresolved positive-MRR client links or prices make the observation invalid and leave the previous valid total in place. Sync exceptions are recorded as failed observations where database access remains available. Existing onboarding and payout behaviors are otherwise preserved. Snapshot UI freshness warning is 36 hours. The historical cash-attribution descriptions above describe the retired Overview; the new Overview has no payout-transaction/listing dependency.

## 2026-09-14 — Future GHL signup subscription handoff (cutover in progress)

`workers/assembly-payment` now supports authenticated `POST /ghl/subscription-paid` with `contact_id`, in addition to the unchanged invoice endpoint. It reads GHL transactions, orders, subscription details and contact data itself. The new path only accepts the configured five Live payment-link IDs, matching Standard product quantities/prices ($350/listing/month plus $150 once), an initial paid Stripe invoice, and an active subscription with `collection_method=charge_automatically` and the paid card as its default method. The matching internal `rf-subscription-agreement-qN-signed` tag is required; only a completed-agreement workflow should add it. Test, refunded, partial, unrelated, manual-invoice, trial and renewal payments cannot provision. The existing email-keyed Durable Object re-verifies the same payment before each attempt and preserves Assembly/Hub deduplication and no-invite behavior.

Native Test checkout established $850 first payment / $700 monthly for two listings, with an actual Stripe Test subscription and saved card. The synthetic subscription was subsequently canceled and its Stripe snapshot confirmed canceled. The adapter has 27 tests plus invoice/subscription runtime concurrency checks. Root and worker typechecks and deployment dry-run passed. Worker version `97be5fd2-91a5-426a-91be-c8a6b6fce368` is deployed; health and Test rejection passed.

This is **not yet a public signup cutover**: native agreement routing and new workflows remain to be connected and validated. New Live links exist but are not linked from the signup form. Existing client recurring invoice schedules were not changed; their later migration is a separate user-requested phase. Do not describe the future signup flow as live until the remaining GHL cutover is verified.

### 2026-09-14 — Future signup Stripe subscription cutover published
Supersedes the earlier in-progress checkpoint. Federico requested future signups first; Danielle and Natasha's existing recurring invoice schedules remain unchanged. All five existing public Live agreement IDs are retained, with same-tab redirects to native Live Payment Links: Q1 6aa82dfa32f95ae35594a897; Q2 6aa82e3732f95ae35594a898; Q3 6aa82ea232f95ae35594a899; Q4 6aa82f05ceb12d9fc1a8cce0; Q5 6aa82f62ceb12d9fc1a8cce3. First payment $350/listing + $150 once; $350/listing monthly Stripe subscription thereafter, saved card, charge_automatically. Booking redirect remains calendar s2jDCEAg86oW89dfOPup (Emily currently).

GHL derives agreement invoice type from product type on reload. Each agreement table therefore uses the new **one-time** First Month Service summary product 6aa8428e3b4b7ff298986122 ($350, fixed quantity 1–5), plus setup. Legal text still states recurring terms. Original recurring checkout catalog/price is unchanged. Agreement settings: One Time; Direct Payment, Send Invoice and auto payment OFF. GHL retains an unsent first-payment draft invoice from the document table; do not send/collect it. Stripe checkout owns real payment and renewal. Existing generated invoices/documents/schedules are not converted.

Published completed-agreement workflows set exact qN signature tags: Q1 3127d7fb-8b0e-4706-aecb-f9948ceb083f; Q2 f46500ad-a2dd-4028-8229-81b4481204d0; Q3 ff1fa091-79ef-4c07-a5c8-1a8b4d4068b0; Q4 59e84b67-e863-4510-976a-182167c7df90; Q5 a8532134-3a73-4dcf-8cf4-c84a434babe9. Published payment-success workflow acf3b37d-22d0-4c9d-b2b4-5281295546c2 waits 1 minute then calls authenticated /ghl/subscription-paid. Worker independently verifies allowed Live source, exact amount/products/quantity, matching signed tag and initial paid Stripe invoice, active subscription, saved card/automatic collection; old invoice endpoint remains. Assembly + Hub dedupe and sendInvite=false retained. Completion adds rf-subscription-initial-paid, which triggers published booking workflow 5cf82464-e160-4a2d-817a-e1b0746bce44 with immediate/day-1/day-3 emails and existing booked/complete/paused guards. All seven published statuses read back via official GHL API.

Use the same email for signing and checkout: native agreement redirect does not expand contact merge tokens. Clean static checkout URLs avoid literal placeholder text. Native Test checkout proved real Stripe subscription behavior; no real-card Live charge was performed. Synthetic Test subscription canceled; nonbinding QA template 6aa83070d0b49900958e6d8f unpublished; unpaid QA invoice 6aa834868fb034ca19fac5e0 deleted (Draft cannot be voided), GET 404 confirmed; paid Test history retained. No real client messages in this cutover. Full operational record: GoHighLevel workspace projects/revfactor/future-stripe-signups-2026-09-14.md. Worker deployed version 4efd1c29-b2d0-4725-87bb-191ff2290a81; implementation commits c64cbf1 and 29b41fa are on main.

## 2026-09-16 — Native GHL signup enrichment

`workers/assembly-payment/src/enrichment.ts` maps verified paid identities into Hub personal display name, separate `business_name`, phone, stable `ghl_contact_id`, PMS/Vrbo, and allowlisted `ghl_onboarding` preparation/booking JSON. Migration `20260916180000_ghl_client_enrichment.sql` supplies a service-role-only transactional RPC: it locks the client, checks email/Assembly/GHL identity, atomically attaches the existing Stripe customer through `client_stripe_customers`, sets the primary pointer and verified automatic-collection flag, and seeds only a missing subscription mirror. Existing Stripe sync remains authoritative for mirror refresh. No Stripe mutation or payment request is made.

Initial enrichment runs inside the existing paid handoff before completion tags. Cloudflare's 15-minute cron refreshes up to ten mapped `onboarding` clients, oldest attempt first; errors rotate through the queue and persist in `ghl_sync_error`. It reads the current booking/assigned host rather than hardcoding Emily. Appointment detail timestamps preserve their explicit timezone offsets. Authenticated `POST /ghl/enrich` (`contact_id`) refreshes a previously completed paid-client Durable Object without reopening provisioning; it is the targeted backfill/retry path. Invoice clients retain their existing billing associations unless subscription evidence exists in their original verified job.

Unknown/In progress answers stay explicit in the profile; they never become verified No. Readiness is client-reported, not proof of co-host access. An Airbnb URL is retained only for a validated public numeric listing URL with query data stripped; no placeholder listing rows, PriceLabs identities, completed audits, contract duration, portal invitations, or active-client status are inferred. Agreement evidence is the existing completed-agreement workflow tag, not a newly guessed document ID. This profile does not create a competing `onboarding_runs` journey. Client detail exposes preparation and business/contact fields under existing client access; billing remains super_admin-only.

Verification: 31 worker tests, invoice/subscription runtime concurrency, scheduled-handler and cancellation/host-refresh tests, root/worker typechecks, dry-run, and transactional database replay/conflict/permission checks (rolled back synthetic records) passed. Roll back worker to `4efd1c29-b2d0-4725-87bb-191ff2290a81` and remove its cron to stop enrichment; additive columns can stay. Historical subscriptions and payments are untouched.

Production Worker enrichment release: `c8d0e8dc-3edd-46cf-9be9-b869593cfce8` (September16). Database migration20260916180000 applied and registered; targeted refresh of the existing paid client verified identity, preparation/current host, unique Stripe link and automatic billing without creating billing or outreach. Central GHL release record contains exact client evidence.


## 2026-09-16 — Signed GHL contract copy for new paid clients

After a new GHL initial subscription payment and successful Assembly/Hub provisioning, `workers/assembly-payment/src/contracts.ts` archives the native signed agreement PDF in that client's existing Assembly company file channel, under `Signed Contracts/RevFactor Service Agreement - <GHL document ID>.pdf`. It copies the original bytes, including GHL's signature certificate; this is a Files item, not a new signature request or an Assembly native Contract.

Eligibility is forward-only: a brand-new email-keyed Durable Object, verified initial payment at/after `CONTRACT_FILES_ACTIVATED_AT` (2026-09-16T15:05:52Z), and a newly created Assembly client. Old jobs, old payments, invoice clients, and pre-existing Assembly clients do not receive a copy. No backfill endpoint was added. Existing billing, booking, invitations and Hub enrichment remain unchanged.

Source: supported GHL document listing plus the exact contact's native signing-completion email retrieved through Conversations. Require completed agreement, matching quantity/name/location/signer/email/revision and signing link. Its native PDF download must redirect to the same document/location in GHL's GCS bucket. Assembly channel membership is checked; upload goes only to the returned HTTPS S3 URL. Native PDF download was verified read-only against a real completed agreement; never store provider URLs/payloads in docs or logs.

Transfers have a separate six-attempt exponential retry budget after provisioning completes. Durable create intents and deterministic paths prevent duplicate folder/file creation; uncertain writes reconcile or enter review. Completion requires Assembly status plus readback SHA-256 equality. Errors preserve provisioning and booking success, expose a safe `contractCopy` status on authenticated `/status`, and add `rf-contract-copy-review`. Federico/the Hub maintainer owns review. `CONTRACT_FILES_ENABLED=false` pauses transfers; `ENABLED=false` pauses the whole handoff. Rollback baseline: Worker c8d0e8dc-3edd-46cf-9be9-b869593cfce8; preserve all copied files and billing records.

Validation: 42 unit tests, four isolated Cloudflare runtime modes (invoice, historical subscription, eligible new contract copy, existing-client exclusion), root/Worker typechecks and dry run. Concurrent/replayed deliveries produce only one byte-identical copy; identity/hash conflicts fail closed. The first naturally occurring production upload remains to be observed. No live QA client or existing-client upload was used.

Production Worker version `f6e1bdd9-1075-4039-8ff6-d5b79fd4e924` deployed September 16, 2026. Health confirms the archive flag and cutoff. Read-only status of a pre-existing completed client has no contractCopy state; all five Live agreement names/public flags were independently confirmed through the supported templates API.
