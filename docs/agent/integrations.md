...
The August 21, 2026 discovery in `docs/event-intelligence-design.md` proposes a replaceable source-adapter layer. PredictHQ is the first implemented beta adapter; the remaining sources below are still candidates.

- PredictHQ: the live trial sample used Events, Suggested Radius (`industry=accommodation`), and Demand Surge over a 90-day window. Store its token only as a server-side secret. Do not use Demand Surge alone: it returned no surge dates for Park City or Gatlinburg/Smokies while the Events feed contained material destination events. Use raw event changes, accommodation impact days, market type, and booking vulnerability.
- Ticketmaster Discovery: candidate source for structured ticketed events. Its public API key must remain server-side for Hub ingestion even though the provider transmits it as a query parameter. It is enrichment, not complete market coverage.
- National Weather Service: candidate authoritative US alert source for watches, warnings, advisories, and unwind/review signals. It does not cover non-US listings.
- Official RSS/Atom/press sources plus GDELT/news discovery: candidate long-lead announcement layer. News items must be corroborated against authoritative evidence before an action gate; Google News RSS may be tested only as a disposable adapter, not a canonical dependency.
...
- `/market-signals` is permission-gated, but market setup has no human activation step. All five configured pilots are active and all coordinate matches are approved in agent mode: Washington 3, Sevierville/Pigeon Forge/Gatlinburg 40, Tucson 7, Myrtle Beach 1, and Park City 1. Once the server runtime has both credentials, the ingestion job automatically enables each registered PredictHQ source before syncing. Human approval begins at the revenue recommendation/action boundary.
...
- `GET /api/cron/market-signals` requires `Authorization: Bearer $CRON_SECRET` and is scheduled once per minute in `vercel.json`. Each invocation enqueues only active agent-managed markets whose source cadence is due, then drains leased jobs (one by default, configurable up to ten within a 300-second budget). Manual editor refreshes enqueue high-priority jobs instead of blocking a Server Action.
...
- The daily PriceLabs cron enqueues `inventory_refresh` for every active managed market after its listing/report work. These jobs recalculate vulnerability and fill cached briefs without calling PredictHQ, so the 90-day provider beta can be removed or allowed to expire without disabling inventory-only rescoring.
- Ticketmaster, NWS, GDELT/news, and official-feed adapters remain unwired. There is still no external notification, Adjustment mutation, PriceLabs write, PMS write, or OTA write.
