...
test('ambiguous client create reconciles existing client by exact email',async()=>{const f=fixture('client_pending');f.s.companyId='company1';f.s.createdAssemblyClient=true;f.p.find=async()=>[{id:'reconciled',email:job.email,companyId:'company1'}];await advance(f.s,f.p);assert.deepEqual(f.calls,['hub','mark']);assert.equal(f.s.clientId,'reconciled');});
...
test('reused existing clients never qualify as newly created',async()=>{const f=fixture();f.p.find=async()=>[{id:'existing',email:job.email}];await advance(f.s,f.p);assert.equal(f.s.createdAssemblyClient,undefined);});
test('lost client-create response cannot reconcile a foreign company',async()=>{const f=fixture('client_pending');f.s.companyId='company1';f.s.createdAssemblyClient=true;f.p.find=async()=>[{id:'existing',email:job.email,companyId:'other'}];await assert.rejects(advance(f.s,f.p),/company_conflict/);});
