...
const softwareColor: Record<string, string> = {
  ...
  "OTA Extranet": "bg-amber-500/10 text-amber-700 border-amber-300 dark:text-amber-400",
  PMS: "bg-violet-500/10 text-violet-700 border-violet-300 dark:text-violet-400",
  Google: "bg-sky-500/10 text-sky-700 border-sky-300 dark:text-sky-400",
...
function CredentialFormDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = isEdit
      ? await updateTeamCredential(credential.id, input)
      : await createTeamCredential(input)
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ? "Update the shared login for this team app."
              : "Add a shared login for an app the team uses."}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="team-cred-name">Name</Label>
              <Input
                id="team-cred-name"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="e.g. PriceLabs main"
                required
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="team-cred-software">Software</Label>
              <Select value={software} onValueChange={setSoftware} required>
                <SelectTrigger id="team-cred-software">
          ...
          <div className="space-y-2">
            <Label htmlFor="team-cred-email">Email / Username</Label>
            <Input
              id="team-cred-email"
              value={email}
          ...
          <div className="space-y-2">
            <Label htmlFor="team-cred-password">Password</Label>
            <Input
              id="team-cred-password"
              value={password}
          ...
          <div className="space-y-2">
            <Label htmlFor="team-cred-notes">Notes</Label>
            <Input
              id="team-cred-notes"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="e.g. 2FA enabled, codes go to shared phone"
            />
          </div>
          <DialogFooter>
...
export function TeamCredentials({
  ...
  async function handleDelete() {
    if (!deleteId) return
    setDeleting(true)
    const result = await deleteTeamCredential(deleteId)
    setDeleting(false)
    setDeleteId(null)
    if (result.error) {
      toast.error(result.error)
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Shared logins for tools the team uses
          </p>
        </div>
        {canCreate && (
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {showActions && <TableHead className="w-[70px]" />}
              </TableRow>
            </TableHeader>
            <TableBody>
