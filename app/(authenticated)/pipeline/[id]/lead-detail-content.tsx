...
import type { StageEventWithProfile } from "./lead-detail"

// Shared server component: fetches everything the lead detail needs and
// renders LeadDetail. Used by the full page (/pipeline/[id]) and by the
// intercepted modal route (@modal/(.)pipeline/[id]).
export async function LeadDetailContent({
  id,
  variant = "page",
}: {
  id: string
  ...
  const [{ data: profiles }, { data: notes }, { data: stageEvents }, canControl] =
    await Promise.all([
      supabase
        .from("profiles")
      ...
      supabase
        ...
        .select(
          "*, changed_by_profile:profiles(full_name, email, avatar_url)"
        )
        .eq("lead_id", id)
  ...
  return (
    <LeadDetail
      ...
      stageEvents={(stageEvents ?? []) as StageEventWithProfile[]}
      variant={variant}
    />
  )
}
